...
def setOsFunc(request):
    # print(request.GET)
    if "favorite_os" in request.GET:    # get 방식의 요청 값이 "favorite_os" 인 경우 처리 
        print(request.GET["favorite_os"])
        # request.session['세션키']
        request.session['f_os']=request.GET["favorite_os"]  # 세션 생성
        return HttpResponseRedirect("/showos")  # redirect 방식 (클라이언트 통해서 서버에 요청)
    else:
...
def showOsFunc(request):
    ...
    if "f_os" in request.session:   # session 값 중에 "f_os" 유무
        ...
        dict_context['sel_os']=request.session['f_os']  # 유효시간 30분
    ...
    # set_expiry(0) 하면 브라우저가 닫힐 떄 세션이 해제됨
