...
# 모델에서 확인한 결과 newspaper : p-value > 0.05이므로 newspaper는 독립변수에서 제외

# 예측 : 새로운 tv값으로 sales를 예측
x_new2 = pd.DataFrame({'tv':[222.2, 55.5, 100.0], 'radio':[30, 40, 50]})
pred2 = lm_mul.predict(x_new2)
print('예측값2 : ', pred2.values)
...
plt.show()  # 평형하지 않고 곡선 형태를 보이므로 선형성을 만족하지 못함. 다항회귀 추천
