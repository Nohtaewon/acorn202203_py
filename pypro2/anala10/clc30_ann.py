...
# Perceptrion (단충 신경망 - 뉴런(node)을 1개 사용)
# input data(feature) * weight(가중치) + bias(편향) 의 합에 대해 임계값을 기준으로 output(출력값)을 보냄
# y = wx+b 라고 하는 일차방정식을 사용
# 논리회로 분류를 위한 간단한 실습
...
ml =Perceptron(max_iter=1000, eta0=0.1).fit(feature, label)    # max_iter=학습횟수, eta0=학습량
print(ml)
