...
# 분류 모델
dt_clf = DecisionTreeClassifier()   # 얘 이외에 다른 분류 모델 클래스를 사용해도 됨.
print(dt_clf)
...
print('분류 정확도:', accuracy_score(train_label, pred)) # 찝찝... 과적합!
...
print('예측 값:', pred2)
print('실제 값:', y_test)
print('분류 정확도:', accuracy_score(y_test, pred2))     # 0.95555 포용성을 위한 모델 완성
# 예) 꼬리가 없는 댕댕이도 댕댕이로 분류할 수 있어야 함
...
print('과적합 방지 처리 방법2-1 : 불균등한 분포를 가진 레이블 데이터에 대한 교차검증 -----')    
from sklearn.model_selection import StratifiedKFold
# 예) 편향, 왜곡된 대출 사기 데이터 : 대부분 정상, 극히 일부만 사기, 이메일(극히 일부만 스팸), 강우(일부만 비옴), 
...
print('평균 검증 정확도 : ', np.round(np.mean(score), 3))  # 0.953
...
grid_dtree.fit(x_train, y_train)    # 자동으로 복수 개의 내무 모형을 생성하고 최적의 파라미터를 제시
...
print('GridSearchCV 최고 정확도:', grid_dtree.best_score_)   # 0.9416
