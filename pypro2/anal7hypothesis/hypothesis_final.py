...
# 각 부서와 직원평가점수 간에 관련이 없다.

print('--------평균차이분석(t-test) : 10, 20번 부서(범주형)와 평균연봉(연속형) 값의 차이가 있는가? ----------')
...
print('평균 : ', np.mean(buser10), ' ', np.mean(buser20))     # 5414.28  4908.33
t_result = stats.ttest_ind(buser10, buser20)
print(t_result)
# Ttest_indResult(statistic=0.4585177708256519, pvalue=0.6523879191675446)
# 해석 : pvalue=0.65238 > 0.05 귀무가설 채택
# 두 부서 간 연봉 평균에 차이가 없다.

print('--------분산분석(ANOVA) : 각 부서(범주형. 요인 1개에 4그룹이 존재)와 평균연봉(연속형) 값의 차이가 있는가? ----------')
# 귀무 : 4개의 부서 간 연봉 평균에 차이가 없다.
# 대립 : 4개의 부서 간 연봉 평균에 차이가 있다.
...
print(f_sta, pv)    # p: 0.7454421884076983 >0.05 귀무채택
# 4개의 부서 간 연봉 평균에 차이가 없다.

# 방법2
lm = ols('jikwon_pay ~ C(buser_num)', data=df3).fit()
