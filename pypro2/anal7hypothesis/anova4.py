...
# 태아수, 관측자 수에 따른 태아의 머리둘레 데이터 사용
# 귀무 : 태아수, 관측자 수는 태아의 머리둘레 평균과 관련이 없다.
# 대립 : 태아수, 관측자 수는 태아의 머리둘레 평균과 관련이 있다.
...
# 교호(상호)작용 처리함
ols2 = ols("머리둘레 ~ C(태아수) + C(관측자수) + C(태아수):C(관측자수)", data=data).fit()
result2 = sm.stats.anova_lm(ols2, typ=2)
print(result2)
# 해석 : p-value:3.295509e-01 > 0.05 이므로 귀무가설 채택
# 태아수, 관측자 수는 태아의 머리둘레 평균과 관련이 없다.
