...
# 해석 : pvalue=0.2525 > 0.05 이므로 귀무가설 채택
# 남녀 두 집단 간 파이썬 시험의 평균 차이가 없다.

print('----------------실습2-----------------------------')
# 실습) 두 가지 교육방법에 따른 평균시험 점수에 대한 검정 수행 two_sample.csv'
# 귀무 : 두 가지 교육방법에 따른 평균시험 점수가 차이가 없다.
# 대립 : 두 가지 교육방법에 따른 평균시험 점수가 차이가 있다.
...
print(stats.shapiro(sco1).pvalue)   # 0.367 > 0.05 정규성 만족
print(stats.shapiro(sco2).pvalue)   # 0.671 > 0.05 정규성 만족
...
# result = stats.ttest_ind(sco1, sco2, equal_var = False)  # 등분산성 만족이 아닌경우
print(result)
