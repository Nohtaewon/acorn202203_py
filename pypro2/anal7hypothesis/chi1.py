...
print()
# 귀무 가설(H0, 영가설) : 벼락치기 공부하는 것과 합격 여부는 관계가 없다.
# 대립 가설(H1, 연구가설) : 벼락치기 공부하는 것과 합격 여부는 관계가 있다.
...
# 결론 : 카이제곱 검정 통계량 값 3 < 임계값 3.84이므로 귀무가설 채택열 내에 있으므로 귀무가설을 채택함
# 벼락치기 공부하는 것과 합격 여부는 관계가 없다. 라는 결론을 얻음
...
# 결론 : 유의수준(α): 0.05 < 유의확률(p-value):0.5578254003710748 이므로 귀무가설을 채택하고 대립가설 기각함
# 카이제곱값과 p-value 는 반비례
