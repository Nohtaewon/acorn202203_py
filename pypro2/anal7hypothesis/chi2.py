# 교차분석
# 적합도(선호도) 검정 : 관측값 들이 어떤 이론적 분포를 따르는지 검정. 요인이 1개 - 일원카이제곱검정. 교차분할표 X
...
# 결론 : p-value:0.01439 < 0.05 이므로 귀무가설을 기각하고 대립가설을 채택. 주사위는 게임에 적합하지 않다.
# 다시말해 사용된 data 값은 우연히 발생한 것이 아니라 어떠한 이유에 의해 필연적으로 발생된 값을 의미함.
# 결론2 : 유의수준 0.05, 자유도 : N - 1 : 5, 임계값 : 11.07
# 임계값 : 11.07 < 통계량:14.20000 귀무가설을 기각하고 대립가설을 채택

# 일원카이제곱검정은 기대되는 빈도와 의미있게 차이가 있는 지를 검증하기 위해 사용!
...
# 결론 : pvalue=0.000399 < 0.05 이므로 귀무가설 기각. 스포츠 음료에 대한 선호도에 차이가 있다.
# 그러므로 향후 어쩌구 저쩌구 .. 조치를 취할 수 있다.
