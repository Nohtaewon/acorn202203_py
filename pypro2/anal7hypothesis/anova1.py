...
# 전제 조건
# - 독립성 : 각 집단은 서로 독립
# - 정규성 : 각 집단은 정규 분포를 따라야 함
# - 불편성 : 등분산성을 갖춰야 함

# 집단 간 분산이 집단 내 분산 보다 충분히 큰 것인가를 파악하는 것
...
print('평균1 : ', score1.mean())  # 67.384
print('평균2 : ', score2.mean())  # 68.357
print('평균3 : ', score3.mean())  # 68.875

lm = ols("data['score']~ data['method']", data=data).fit()
result = sm.stats.anova_lm(lm, typ=1)
print(result) # p-value : 0.727597
# 해석 : p-value : 0.727597 > 0.05 귀무가설 채택
# 교육방법에 따른 실기시험 평균점수에 차이가 없다.

print('사후검정 ------')
# ANOVA는 그룹 간의 평균값 차이의 유의함을 판단해 준다.
# 그런데 각 그룹의 평균값의 차이는 설명하지 않음
# 그래서 사후검정(Post Hoc Test)을 실시함
