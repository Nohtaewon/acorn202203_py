...
# 원래는 핸들을 별도의 파일로 만들고 완성차 클래스 만들 때 
# 호출해야 하나 편의상 아래에 완성차 클래스를 작성하기로 하자.
...
class PohamCar:
    ...
    def play_turnhandle(self, q):
        if q>0:
            self.turnShow_msg=self.handle.right_turn(q)
        elif q<0:
            self.turnShow_msg=self.handle.left_turn(q)
        elif q==0:
...
if __name__=='__main__':
    tom=PohamCar('톰')
    tom.play_turnhandle(10)
    print(tom.ownerName+'의 회전량은'+tom.turnShow_msg+str(tom.handle.quantity))  
        
    tom.play_turnhandle(0)
    print(tom.ownerName+'의 회전량은'+tom.turnShow_msg+str(tom.handle.quantity))     
        
    print()
    james=PohamCar('제임스')    
    james.play_turnhandle(-5)
    print(james.ownerName+'의 회전량은'+tom.turnShow_msg+str(tom.handle.quantity))     
