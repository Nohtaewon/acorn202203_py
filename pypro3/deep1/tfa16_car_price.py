...
y_train = train_df[['가격']]  # 가격 열을 label
y_test = test_df[['가격']]
...
print(set(x_train.종류))  # {'중형', '대형', '준중형', '소형'}
print(set(x_train.연료))  # {'LPG', '디젤', '가솔린'}
print(set(x_train.변속기)) # {'자동', '수동'}

# 범주형 칼럼을 dummy 화 : LabelEncoder, OneHotEncoder
# make_column_transformer를 사용하여 특정 열에만 OneHotEncoder 적용
transformer = make_column_transformer((OneHotEncoder(), ['종류', '연료', '변속기']),
                                      remainder='passthrough')
# remainder='passthrough' or 'drop' :  passthrough 지정된 열 이외의 모든 열이 transformer로 전달
transformer.fit(x_train)
# '종류', '연료', '변속기' 세개의 칼럼이 참여해 원핫처리됨 [[0. 0. 1. 0. 0. 1. 0. 0. 1.].....
