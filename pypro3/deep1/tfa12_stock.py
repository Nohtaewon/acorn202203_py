...
xy = scaler.fit_transform(xy)   # scaler.inverse_transform(xy)
print(xy[:2])
...
print('train/test를 안한 evaluate: ', model2.evaluate(x_test, y_test))
...
print('train/test 안한 r2_score:', r2_score(y_test, pred2))   # 과적합 의심
...
print('train/test => 후  evaluate', model3.evaluate(x_test, y_test))

# 결정계수
pred3 = model3.predict(x_test)
print('train/test => 후 r2_score:', r2_score(y_test, pred3))

# 머신러닝의 이슈 : 모델 성능에 있어, 최적화 일반화 사이의 줄다리기
